   ...
   return {
      ...
      callback(request, response, next) {
         return _asyncToGenerator(function* () {
            ...
            const remoteResponse = yield fetch(`https://${shop}/admin/oauth/access_token`, {
               method: 'POST',
               headers: {
            ...
            const responseBody = yield remoteResponse.json();
            const accessToken = responseBody.access_token;

            try {
               const { token } = yield shopStore.storeShop({ accessToken, shop });
